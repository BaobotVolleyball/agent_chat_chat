...
class CohereReranking(BaseReranking):
    ...
    def run(self, documents: list[Document], query: str) -> list[Document]:
        ...
        if not self.cohere_api_key and self.use_key_from_qcom:
            try:
                from qcom.embeddings.manager import (
                    embedding_models_manager as embeddings,
                )

                cohere_model = embeddings.get("cohere")
                qcom_cohere_api_key = cohere_model._kwargs.get(  # type: ignore
                    "cohere_api_key"
                )
                if qcom_cohere_api_key != "your-key":
                    self.cohere_api_key = qcom_cohere_api_key
            except Exception as e:
                print("Cannot get Cohere API key from `qcom`", e)
